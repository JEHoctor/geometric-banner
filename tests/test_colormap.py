...
from geometric_banner.colormap import VIRIDIS, viridis
...
def test_matches_matplotlib_anchor_values(x, expected):
    assert viridis(x) == expected


def test_top_of_range_maps_to_last_entry():
    assert viridis(1.0) == VIRIDIS[-1]
    assert viridis(0.999) == VIRIDIS[-1]


@pytest.mark.parametrize("x", [-0.01, 1.01])
def test_rejects_out_of_range(x):
